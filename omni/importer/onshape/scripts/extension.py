...
from omni.importer.onshape import SETTINGS_PATH
...
def cache_imports():
    ...
    from ..widgets.content_widget import OnshapeContentWidget
    from ..widgets.documents_widget import DocumentListDelegate


threading.Thread(target=cache_imports).start()
...
class OnshapeImporter(omni.ext.IExt):
    ...
    def on_startup(self, ext_id: str):
        ...
        self._hooks.append(
            manager.subscribe_to_extension_enable(
                ...
                ext_name="omni.importer.onshape",
                hook_name="omni.importer.onshape omni.kit.window.preferences listener",
            )
        )
    ...
    def on_element_selected(self, item):
        from ..widgets.assembly_widget import (
            AssemblyDetailsWidget,
            OnshapeAssemblyModel,
        )
    ...
    def on_stage_imported(self, stage_path):
        if not self.prim:
            ...
            Usd.ModelAPI(self.prim).SetKind(Kind.Tokens.assembly)
